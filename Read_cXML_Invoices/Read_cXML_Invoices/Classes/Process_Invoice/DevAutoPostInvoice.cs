...
{
    public class DevAutoPostInvoice : AutoPostInvoice
    {
        public override void AutoPost(Ship ship, InvoiceHeader invoice)
        {
            try
            {
                ...
                //if (invoice.PDFFileName.Length > 0 && invoice.Errors.Count == 0) TagKwiktagDocument(ship, invoice);
                if (invoice.Errors.Count == 0 && invoice.PurchaseOrderNo.Length > 0)
                    PostInvoice(ship, invoice);
            }
            catch (Exception ex)
            {
                invoice.Errors.Add(ex.Message);
                Constants.ERRORS.Add(new Error(ex, new System.Data.SqlClient.SqlCommand($"Invoice ID = '{invoice.InvoiceID}', Order ID = '{invoice.OrderID}'"), "AutoPostInvoice_DevZAutoPost", "AutoPost"));
            }
        }
        ...
        private void ValidateInvoicePurchaseOrder(Ship ship, InvoiceHeader invoice)
        {
            ...
            {
                DevZAutoPostDoc.AutoPostDocument auto = new DevZAutoPostDoc.AutoPostDocument();
                auto.UseDefaultCredentials = true;

                DevZPurchaseOrder.PurchaseOrder_Service poservice = new DevZPurchaseOrder.PurchaseOrder_Service();
                poservice.UseDefaultCredentials = true;

                DevZPurchaseOrder.PurchaseOrder po = poservice.Read(invoice.Document_ID);
                if (po == null)
                    invoice.Errors.Add("Purchase Order for Invoice could not be found in Navision.");
                else
                {
                    ...
                    foreach (var line in invoice.Lines)
                    {
                        ...
                        DevZPurchaseOrder.Purchase_Order_Line pline = null;
                    ...
                    if (invoice.Errors.Count == 0)
                    {
                        if (po.Status == DevZPurchaseOrder.Status.Released)
                        {
        ...
        private void UpdatePurchaseOrder(InvoiceHeader invoice)
        {
            ...
            DevZPurchaseOrder.PurchaseOrder po = poservice.Read(invoice.PurchaseOrderNo);
            ...
            foreach (var line in invoice.Lines)
            {
                DevZPurchaseOrder.Purchase_Order_Line pline = null;
                foreach (var pl in po.PurchLines)
                    if (pl.Type == DevZPurchaseOrder.Type.Item && pl.Line_No == line.PurchLine_LineNumber)
                        pline = pl;

                if (pline != null)
                {
                    if (pline.Quantity_Received < line.Quantity)
                    {
